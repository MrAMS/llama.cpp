...
const void* loadModel() {
    ...
    NSString *pathString = [documentsURL.absoluteString stringByAppendingString:@"coreml_minicpmv45_vit_f16.mlmodelc"];
    NSURL *modelURL = [NSURL URLWithString:pathString];

    NSLog(modelURL.absoluteString);

    const void* model = CFBridgingRetain([[coreml_minicpmv45_vit_f16 alloc] initWithContentsOfURL:modelURL error:nil]);
    return model;
}
...
void predictWith(const void* model, float* embed, float* encoderOutput) {
    ...
    coreml_minicpmv45_vit_f16Output *modelOutput = [(__bridge id)model predictionFromInput:inMultiArray error:nil];
